...
Returns tests left claimed by an interrupted shard to the queue. A test is claimed the moment a shard takes it and settled when its result is written, so a shard killed between those two points leaves its tests unreachable to every other shard for the rest of the run.

| Option            | Description                                                    | Type     | Default | Required? |
| ----------------- | -------------------------------------------------------------- | -------- | ------- | --------- |
| `--run-id`        | Run ID generated by `create` command                           | `string` | -       | yes       |
| `--stale-minutes` | How long a test may stay claimed before it counts as abandoned | `number` | `10`    | no        |

Only storages that track a per-test status support this: `pg`, `mysql`, and `mongo`. `file`, `redis`, and `dynamo-db` claim a test by removing it from the queue and keep no record of what was taken, so there is nothing left to reset.

The claim timestamp is stamped when the test is handed out and is **not** refreshed while it runs, so `--stale-minutes` has to be larger than your longest single test. Set it too low and a test that is merely slow gets handed to a second shard and runs twice.

Running it once after every shard has exited avoids that trade-off entirely — in GitHub Actions, a job that `needs` your shard matrix and runs `if: always()`. At that point nothing can legitimately still be claimed, so `--stale-minutes 0` is exact rather than a guess:

```bash
npx playwright-orchestrator cleanup pg --run-id $RUN_ID --stale-minutes 0
```
...
- ✅ Restore unfinished tests in case shard terminated — see [`cleanup`](#cleanup), for storages that track a per-test status
